...
class UploadedFile(db.Model):
    __tablename__ = "uploaded_file" # Explicit table name
    id = db.Column(db.Integer, primary_key=True)
    ...
    jd_entry = db.relationship("JobDescription", backref="source_uploaded_file", uselist=False, foreign_keys="JobDescription.uploaded_file_id")
    # If a CV is an UploadedFile, it can be linked to a CVEntry
    cv_entry = db.relationship("CVEntry", backref="source_uploaded_file", uselist=False, foreign_keys="CVEntry.uploaded_file_id")
...
class JobDescription(db.Model):
    __tablename__ = "job_description" # Explicit table name
    id = db.Column(db.Integer, primary_key=True)
    user_id = db.Column(db.Integer, db.ForeignKey("user.id"))
...
class CVEntry(db.Model): 
    __tablename__ = "cv_entry" # Explicit table name
    id = db.Column(db.Integer, primary_key=True)
    user_id = db.Column(db.Integer, db.ForeignKey("user.id"))
    ...
    raw_text_content = db.Column(db.Text, nullable=True)
    processed_file_path = db.Column(db.String(512), nullable=True)  # Path to the (potentially converted) PDF file for LLM analysis
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
...
class AnalysisSession(db.Model):
    __tablename__ = "analysis_session" # Explicit table name
    id = db.Column(db.Integer, primary_key=True)
    ...
    total_cvs_to_analyze = db.Column(db.Integer, default=0)
    cvs_analyzed_count = db.Column(db.Integer, default=0)
...
class AnalysisResult(db.Model):
    __tablename__ = "analysis_result" # Explicit table name
    id = db.Column(db.Integer, primary_key=True)
